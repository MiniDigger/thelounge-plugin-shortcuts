...
const runShortcut = {
  input: function (client, target, command, args) {
    if (doesShortcutExist(command)) {
      ...
      client.sendMessage(
        red + "Did not find shortcut " + code + command + code + ", has it been removed? " + "Removed shortcuts get unregistered on your next restart.",
        target.chan,
        client
      );
    }
...
function handleAdd(client, target, args) {
  ...
  if (doesShortcutExist(from)) {
    if (args[0] === "addf") {
      ...
      client.sendMessage(
        red + "Shortcut " + code + from + code + " does exist, either remove the old one use use " + code + "/shortcut addf <from> <to>" + code,
        target.chan
      );
      client.sendMessage(
        red + "You can also use " + code + "/shortcut addnext <from> <to>" + code + " to add a new command to this existing shortcut",
        target.chan
      );
      return;
    }
...
function sendUsage(client, target) {
  client.sendMessage(red + "Usage: /shortcut <add{,f,next}|list|remove>", target.chan);
}
...
function copy_legacy_db(thelounge, new_path) {
  ...
  try {
    ...
    thelounge.Logger.warn(`failed to copy old db from ${old} to ${new_path}: ${err}`);
  }
}
